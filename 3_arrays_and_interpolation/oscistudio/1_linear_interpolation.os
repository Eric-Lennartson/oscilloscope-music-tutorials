...
"groups": [
	{
		...
		{OsciVectorPluginLivecoding:
			...
			"enabled":1
			"expanded":1
			"title":"Basic Linear Interpolation"
			"bg":0,204,51,255
			"interp_amount":0
			"sourceCode":"
SLIDER(interp_amount, 0..1);

// imprecise method, with the potential for
...
vec3 gen(float t) {
	...
	//v.x = lerp1(start, stop, interp_amount);
	//v.x = lerp2(start, stop, interp_amount);

	// we could implement our own linear interpolation function
	// for two vec3's if we wanted, but oscistudio already has
	// this built in. Just call blend(). Check out the example
	// below to see how that one works.
	...
	return v;
		...
		{OsciVectorPluginLivecoding:
			"uid":"j1lrIWMi3BOBzjLdstSkvSaLzkBV8BDq"
			"enabled":1
			"expanded":1
			"title":"Blend"
			"bg":77,77,77,255
...
vec3 gen(float t){
	...
	// replace interp_amount with t, to turn it into a line
	// between the two points.

	return blend(interp_amount, v1, v2);
}"
		}
		]
		"sample_plugins": [
