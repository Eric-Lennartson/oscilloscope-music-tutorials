...
vec3 gen(float t){
	...
	float t_lerp = mod1(tn);

	// PathRecorder is nice, but if we want to control
	// the interpolation at each point ourselves, we'll
	// have to roll our own.

	return blend(t_lerp*interp_amount, v1, v2);
}"
		}
		]
		"sample_plugins": [
